...
make_vocab('imdb_train_text.txt')

def save_vocab():
	f = open('saved_vocab_pure.txt', 'w')
	for item in vocab:
		f.write("%s\n" % item)
	f.close()
...
clean_and_enumerate('imdb_train_text.txt', 'modified_train_data.txt' )
...
def training():
	...
	file_x=open('modified_train_data.txt','r')
...
with open('theta_q1a.pkl', 'wb') as f:
	pickle.dump(Theta,f)
...
with open('P_ofy_q1a.pkl','wb') as f:
	pickle.dump(P_ofy,f)
...
def testing():
	clean_and_enumerate('imdb_test_text.txt','modified_test_data.txt')
...
# with open('theta.csv', 'rU') as f:
# 	reader = csv.reader(f)
# 	Theta = list(list(rec) for rec in csv.reader(f, delimiter=','))
